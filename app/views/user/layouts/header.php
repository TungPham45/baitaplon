<header class="user-header">
    <div class="header-container">
        ...
        <div class="header-search" style="flex: 1; max-width: 800px; margin: 0 20px;">
            ...
            <form action="/baitaplon/Product/search" method="GET" id="searchForm"
                class="d-flex align-items-center gap-1" style="display: flex; gap: 5px; width: 100%;">

                <input type="hidden" name="danhmuc" id="inputDanhmuc"
                    value="<?php echo isset($data['category']) ? $data['category'] : ''; ?>">
                <input type="hidden" name="diachi" id="nav-input-diachi"
                    value="<?php echo htmlspecialchars($address); ?>">

                <div class="dropdown" style="position: relative;">
                    <button class="btn btn-light dropdown-toggle" type="button" id="dropdownMenuButton"
                        data-bs-toggle="dropdown" aria-expanded="false" style="height: 40px; white-space: nowrap;">
                        <span id="catDisplay"><?php echo htmlspecialchars($currentCatName); ?></span>
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                        style="position: absolute; z-index: 1000; background: #fff; border: 1px solid #ccc; list-style: none; padding: 0; display: none;">
                        <li><a class="dropdown-item" href="#"
                                onclick="selectCategory('', 'Tất cả danh mục'); return false;"
                                style="display: block; padding: 8px 15px; text-decoration: none; color: #333;">Tất cả
                                danh mục</a></li>
                            ...
                            <?php if (!empty($parent['children'])): ?>
                                <li class="dropdown-item-parent" style="position: relative;">
                                    <a class="dropdown-item" href="#"
                                        ...
                                        style="display: block; padding: 8px 15px; text-decoration: none; color: #333; font-weight: bold;">
                                        <?php echo htmlspecialchars($parent['ten_danhmuc']); ?> &raquo;
                                    </a>
                                    <ul class="submenu"
                                        style="display: none; position: absolute; left: 100%; top: 0; background: #fff; border: 1px solid #ccc; width: 200px; list-style: none; padding: 0;">
                                        <?php foreach ($parent['children'] as $child): ?>
                                            <li>
                                                <a class="dropdown-item" href="#"
                                                    onclick="selectCategory('<?php echo $child['id_danhmuc']; ?>', '<?php echo $child['ten_danhmuc']; ?>'); return false;"
                ...
                <input type="text" name="keyword" class="form-control" placeholder="Tìm sản phẩm..."
                    ...
                    style="flex: 1; height: 40px; padding: 0 10px; border: 1px solid #ccc;">

                <div class="dropdown" id="nav-location-dropdown" style="position: relative;">
                    <button class="btn btn-outline-secondary dropdown-toggle" type="button" id="navLocationBtn"
                        data-bs-toggle="dropdown" aria-expanded="false"
                        style="height: 40px; max-width: 150px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; background: #fff; border: 1px solid #ccc;">
                        <?php echo !empty($address) ? htmlspecialchars($address) : 'Toàn quốc'; ?>
                    </button>

                    <div class="dropdown-menu p-2 shadow mt-1" aria-labelledby="navLocationBtn"
                        style="width: 300px; position: absolute; right: 0; z-index: 1000; background: #fff; border: 1px solid #ccc; display: none; padding: 10px;">
                        <div class="input-group mb-2" style="display: flex; margin-bottom: 10px;">
                            <input type="text" class="form-control" id="nav-search-box"
                                placeholder="Nhập Huyện hoặc Tỉnh..." autocomplete="off"
                                style="width: 100%; padding: 5px;">
                        </div>
                        <div class="list-group list-group-flush" id="nav-location-list"
                ...
                <button type="submit" class="btn btn-primary"
                    style="height: 40px; padding: 0 15px; background: #f59e0b; color: #fff; border: none; cursor: pointer;">
