...
<body>
    ...
    <main class="content-container" style="max-width: 1200px; margin: 20px auto; padding: 0 15px;">
        ...
        <div class="user-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 20px;">
            <?php if (!empty($otherUsers)): ?>
                <?php foreach ($otherUsers as $u): ?>
                    <a href="/baitaplon/User/profile/<?php echo $u['id_user']; ?>" class="user-item-card" style="text-decoration: none; color: inherit;">
                        <div class="card-white" style="text-align: center; padding: 20px; transition: 0.3s; cursor: pointer;">
                            <img src="/baitaplon/public/uploads/avatars/<?php echo $u['anhdaidien'] ?: 'default.png'; ?>" 
                            ...
                            <p style="font-size: 0.8rem; color: #666;"><i class="fas fa-map-marker-alt"></i> <?php echo $u['diachi'] ?: 'N/A'; ?></p>
                            <span style="display: inline-block; margin-top: 10px; color: #ff8800; font-size: 0.85rem; font-weight: bold;">Xem hồ sơ &raquo;</span>
                        </div>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
