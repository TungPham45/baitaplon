// Public/js/home_js.js

// Dữ liệu Hành chính (Giữ nguyên của bạn)
const locationData = {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // --- 1. Xử lý Form ĐĂNG TIN (Giữ nguyên logic của bạn, chỉ thêm xử lý địa chỉ) ---
    ...
    if (cat1 && form) {
        ...
        form.addEventListener('submit', function(e) {
            ...
            let isValid = true;
            const errShow = (id) => { document.getElementById('err-'+id).style.display='block'; isValid=false; };
            const errHide = (id) => { document.getElementById('err-'+id).style.display='none'; };

            if(!cat1.value) errShow('catLevel1'); else errHide('catLevel1');
            if(groupCat2.style.display !== 'none' && !cat2.value) errShow('catLevel2'); else errHide('catLevel2');
            if(!document.getElementById('title').value.trim()) errShow('title'); else errHide('title');
            if(!document.getElementById('price').value.trim()) errShow('price'); else errHide('price');
            // Kiểm tra địa chỉ
            if(!document.getElementById('post-address-input').value.trim()) errShow('address'); else errHide('address');

            if(selectedFiles.length === 0) errShow('images'); else errHide('images');

            if(isValid) {
                const formData = new FormData(form);
                selectedFiles.forEach(f => formData.append('images[]', f));
                
                fetch('/baitaplon/PostController/add', { method: 'POST', body: formData })
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
    ...
    // --- 2. Xử lý AUTOCOMPLETE ĐỊA CHỈ (Dùng chung cho cả Search & Post) ---
    
    // Chuẩn bị dữ liệu phẳng: [{name: "Quận 1, TP. Hồ Chí Minh"}, {name: "TP. Hồ Chí Minh"}]
    let flatLocations = [];
    Object.keys(locationData).forEach(prov => {
        flatLocations.push(prov); // Thêm Tỉnh
        locationData[prov].forEach(dist => {
            flatLocations.push(`${dist}, ${prov}`); // Thêm Huyện, Tỉnh
        });
    });

    // Hàm thiết lập gợi ý cho 1 ô input
    function setupAddressAutocomplete(inputId, listId) {
        ...
        input.addEventListener('input', function() {
            ...
            // Lọc danh sách (Tìm Huyện trước, Tỉnh sau)
            const matches = flatLocations.filter(loc => loc.toLowerCase().includes(val)).slice(0, 10); // Lấy tối đa 10 kết quả
            ...
            if (matches.length > 0) {
                ...
                matches.forEach(match => {
                    ...
                    item.innerHTML = `<i class="bi bi-geo-alt"></i> ${match}`;
                    item.onclick = function() {
                        input.value = match;
                        list.style.display = 'none';
                        // Ẩn lỗi nếu có (cho form post)
    ...
    // Áp dụng cho ô Tìm kiếm trên Navbar
    setupAddressAutocomplete('nav-address-input', 'nav-address-list');

    // Áp dụng cho ô Đăng tin trong Modal
    setupAddressAutocomplete('post-address-input', 'post-address-list');
