...
function submitVote() {
    ...
    // --- SỬA ĐOẠN NÀY ĐỂ BẮT LỖI ---
    .then(response => response.text()) // 1. Lấy toàn bộ text về trước (không ép kiểu JSON vội)
    .then(text => {
        console.log("Server trả về:", text); // 2. In ra console (F12) để xem

        try {
            return JSON.parse(text); // 3. Thử chuyển đổi sang JSON thủ công
        } catch (e) {
            // Nếu lỗi tại đây, nghĩa là PHP đang in ra HTML hoặc lỗi (Fatal error)
            throw new Error("Server không trả về JSON hợp lệ. Nội dung lỗi: " + text);
        }
    })
    // -------------------------------
    .then(data => {
        if (data.status === 'success') {
            ...
            alert('Lỗi logic: ' + (data.message || 'Không rõ nguyên nhân'));
        }
    })
    .catch(error => {
        console.error('Chi tiết lỗi:', error);
        // Hiển thị một phần lỗi để bạn dễ sửa
        alert('Lỗi hệ thống: ' + error.message.substring(0, 150) + "..."); 
    });
}
